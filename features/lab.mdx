...
Lab is layer 03 of [the stack](/introduction) — the surface an analyst goes to when there's a question. Type it in English, get back structured analysis: chart, table, narrative, with citations on every number. No template to fill, no dashboard to build, no formula to write. Lab picks the UI the answer needs and renders it inline.
